...
local Model: Model?
local Volcanos = {} :: {[string]: Model}

Network:Bind("SummonVolcano", function(Location, GUID)
	local Volcano = FX.Volcano.Volcano:Clone()
	Volcano.Parent = workspace.Terrain
	
	Volcanos[GUID] = Volcano
	
	Volcano:PivotTo(
		CFrame.new(Location)
	)
	
	for i, v in Volcano:GetDescendants() do
...
Network:Bind("DestroyVolcano", function(GUID)
	local Volcano = Volcanos[GUID]
...
return function(Origin, Destination, FormingSpeed, AttackSize)
	local Fireball = FX.Volcano.Fireball:Clone()
	local Impact = FX.Volcano.Impact:Clone()
	...
	local p0 = Origin + Vector3.new(0, 20, 0)
	...
	EasyRender.new({
		...
		if x >= 1 then
			...
			Debris:AddItem(Fireball, DestructionSpeed + .1)
			Debris:AddItem(Impact, DestructionSpeed + 1)
		end
	end)
end